...
\problem{Write a tail recursive version of the {\it{filter}} function using an
accumulator. The non-tail recursive verions is as follows:
\begin{program**}
\> filter :: (a \arrow Bool) \arrow [a] \arrow [a]\\
\> filter p [] = []\\
...
Folds are rich - they can be used to implement other list functions.  For
example, we can implement reverse using {\tt{foldr}} and {\tt{foldl}} as
follows.
\begin{program**}
\> revr = foldr ($\backslash$y ys \arrow ys ++ [y]) []\\
\> revl = foldl (flip (:)) []\\
\end{program**}

\problem{Use {\it{foldl}} (or {\it{foldr}}) to implement {\it{map}} and {\it{filter}}.
}
...
% comment-column:0
% comment-start: "% "
% compile-command: "pdflatex hw10e"
% fill-column:79
% End:
