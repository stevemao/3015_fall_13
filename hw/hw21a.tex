...
\noindent{} Elements of the syntactic class {\it{term1}} take one of the
following forms: it is an {\it{identifier}}; it is an abstraction which starts
with the keyword {\bf{lambda}}; it is a spread which starts with the keyword
{\bf{spread}}; it is a pairing term which consists of a pair of terms enclosed
in angled brackets or it is a {\it{term}} which is enclosed in parenthesis.

\noindent{}You can build your {\it{identifier}} parser with the parser
{\tt{identifier}} (which is in the file {\em{Parser.hs}}) though you need to
make sure that the keywords {\tt{spread}} and {\tt{lambda}} are {\em{not}}
included among the class of identifiers for this language.

\exercise{Write {\tt{Term}} parsers {\tt{pAbs}} for abstractions (lambda
terms), {\tt{pSpread}} for spread terms, {\tt{pPair}} for pair terms, a parser
{\tt{pParenTerm}}for terms enclosed in parenthesis, and the {\tt{pTerm1}}
parser for the class {\it{term1}}.
}
